...
#hat einige gute kritiken bekommen, hier erstmal flask
...
import logging
from waitress import serve #der server von Flask ist nicht zwingend stable oder secure (doc + warning)
                ...
                for k, v in values.items():
                    if (isinstance(v, int) or isinstance(v, float)) and "_scale" not in k:
                        ...
                        if  k == "power_dc" or k == "power_ac": #starke Reduktion will aber die scale von oben mitnehmen  
                            inverter_data["fields"].update({k: round(float(v * (10 ** scale)),2)})
        ...
        #                    "c_sunspec_did": meter_values["c_sunspec_did"]
                    ...
                    for k, v in meter_values.items():
                        if (isinstance(v, int) or isinstance(v, float)) and "_scale" not in k:
                            ...
                            if k == "power":
                                meter_data["fields"].update({k: round(float(v * (10 ** scale)),2)})
        ...
        #                   "c_sunspec_did": battery_values["c_sunspec_did"]
                    ...
                    for k, v in battery_values.items():
                        ...
                        if k=="soe" or k=="instantaneous_power" or k=="status":
                            battery_data["fields"].update({k: round(v,2)})
                ...
                #with open("/misc/solarlogFull.txt","a") as file:
                #    file.write(current_time+": "+str(json_body) +"\n")
...
def start_server():    
    ...
    #global args bei read unnoetig
    #app.run(host='0.0.0.0', port=args.myport, debug=False, threaded=True) #waitress ist stabiler
    serve(app, host="0.0.0.0", port=args.myport)
...
#@app.before_request
#def log_request():
#    logger.info(f"Anfrage: {request.method} {request.url}")
#    logger.info(f"Headers: {request.headers}")
#    logger.info(f"Daten: {request.data}")
        
# API Route, um Daten bereitzustellen
@app.route('/data', methods=['GET'])
def get_data():
    #global json_body global nur bei änderungen noetig, sinnvoll
    global dataLock
    with dataLock:
        return jsonify(data) # Teil der Daten reicht jsonify(json_body)

@app.route('/dataFull', methods=['GET'])
def get_dataFull():
    global dataLock
    with dataLock:
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--myport", type=str, default=config["myport"],help="port auf  dem dieser Server lauscht")
    ...
    modbus_thread = threading.Thread(target=read_data, daemon=True)
    modbus_thread.start()
